...
var classyDirectivesForm = angular.module('classy.directives.form', []);
...
classyDirectivesForm.directive('input', scBlurFocusDirective).directive('select', scBlurFocusDirective)
			...
			link: function (scope, element, attrs, ctrl) {
				$timeout(function (){
					scope.$watch(attrs.ngModel, function (value) {
						var array = $parse(attrs.scIsInArray)(scope);
		...
		return {
			...
			link: function(scope, element, attrs) {
				...
				return element.bind('submit', function() {
					...
					if (form.$valid) {
						return scope.$apply(attrs.scSubmit);
					}
				});
			}
		};
